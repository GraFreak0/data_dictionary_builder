...
test_mysql.py
=============
Exercises every major feature of data_dictionary_builder against MySQL/MariaDB.
...
Shared / fallback:
    ...
    MYSQL_PASSWORD
    MYSQL_SCHEMAS    comma-separated schemas to test  (default: MYSQL_DB)
...
_MY_PORT     = int(os.getenv("MYSQL_PORT", 3306))
_MY_DB       = os.getenv("MYSQL_DB", "mysql")
_MY_USER     = os.getenv("MYSQL_USER", "root")
_MY_PASSWORD = os.getenv("MYSQL_PASSWORD", "")

# ── Source connection ─────────────────────────────────────────────────────────
SOURCE_CONFIG = {
    "db_type":  CONNECTOR,
    "host":     os.getenv("SOURCE_MYSQL_HOST")     or _MY_HOST,
    "port":     int(os.getenv("SOURCE_MYSQL_PORT") or _MY_PORT),
    "database": os.getenv("SOURCE_MYSQL_DB")       or _MY_DB,
    "user":     os.getenv("SOURCE_MYSQL_USER")     or _MY_USER,
    "password": os.getenv("SOURCE_MYSQL_PASSWORD") or _MY_PASSWORD,
}

# ── Destination connection ────────────────────────────────────────────────────
DEST_CONFIG = {
    "db_type":  CONNECTOR,
    "host":     os.getenv("DEST_MYSQL_HOST")     or _MY_HOST,
    "port":     int(os.getenv("DEST_MYSQL_PORT") or _MY_PORT),
    "database": os.getenv("DEST_MYSQL_DB")       or _MY_DB,
    "user":     os.getenv("DEST_MYSQL_USER")     or _MY_USER,
    "password": os.getenv("DEST_MYSQL_PASSWORD") or _MY_PASSWORD,
}

# TARGET_SCHEMAS is set dynamically at runtime — see __main__
TARGET_SCHEMAS: list = os.getenv("MYSQL_SCHEMAS", _MY_DB).split(",")
...
def test_schema_filter_strategies():
    ...
    _db = TARGET_SCHEMAS[0]
    cases = [
        ("4a. Exact name",   [_db]),
        ("4b. Glob",         [f"{_db[:3]}%"]),
        ("4c. prefix:",      [f"prefix:{_db[:3]}"]),
        ("4d. suffix:",      [f"suffix:{_db[-3:]}"]),
        ("4e. contains:",    [f"contains:{_db[:3]}"]),
        ("4f. regex:",       [f"regex:^{_db}.*$"]),
        ("4g. Mixed",        [_db, "prefix:stg_", "regex:^analytics_\\d{4}$"]),
        ("4h. None  (all)",  None),
    ]
...
def test_metadata_export(helper, dest_db_meta):
    ...
    meta_path = helper.reports_json_dir / "mysql_metadata.json"
...
if __name__ == "__main__":
    ...
    print(f"\n  source host : {SOURCE_CONFIG['host']}:{SOURCE_CONFIG['port']}/{SOURCE_CONFIG['database']}")
    print(f"  dest   host : {DEST_CONFIG['host']}:{DEST_CONFIG['port']}/{DEST_CONFIG['database']}")
    ...
    timer.summary("MySQL Test Suite — Execution Summary")
