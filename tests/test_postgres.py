...
_PG_DB       = os.getenv("PG_DB", "postgres")
...
SOURCE_CONFIG = {
    "db_type":  CONNECTOR,
    "host":     os.getenv("SOURCE_PG_HOST")     or _PG_HOST,
    "port":     int(os.getenv("SOURCE_PG_PORT") or _PG_PORT),
    "database": os.getenv("SOURCE_PG_DB")       or _PG_DB,
    "user":     os.getenv("SOURCE_PG_USER")     or _PG_USER,
    "password": os.getenv("SOURCE_PG_PASSWORD") or _PG_PASSWORD,
}

# ── Destination connection ────────────────────────────────────────────────────
DEST_CONFIG = {
    "db_type":  CONNECTOR,
    "host":     os.getenv("DEST_PG_HOST")     or _PG_HOST,
    "port":     int(os.getenv("DEST_PG_PORT") or _PG_PORT),
    "database": os.getenv("DEST_PG_DB")       or _PG_DB,
    "user":     os.getenv("DEST_PG_USER")     or _PG_USER,
    "password": os.getenv("DEST_PG_PASSWORD") or _PG_PASSWORD,
}

# TARGET_SCHEMAS is set dynamically at runtime — see __main__
TARGET_SCHEMAS: list = os.getenv("PG_SCHEMAS", "public").split(",")
...
def test_schema_filter_strategies():
    ...
    cases = [
        ...
        ("4f. regex:",       [f"regex:^{_s}.*$"]),
        ("4g. Mixed",        [_s, "prefix:stg_", "regex:^analytics_\\d{4}$"]),
        ("4h. None  (all)",  None),
    ]
